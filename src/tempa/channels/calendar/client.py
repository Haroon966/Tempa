    ...
    def create_event(
        ...
        *,
        ...
        insert_kwargs: dict[str, Any] = {
            "calendarId": calendar_id,
            "body": body,
            "sendUpdates": send_updates if attendee_emails else "none",
        }
        if with_meet:
            insert_kwargs["conferenceDataVersion"] = 1
        try:
            created = events_resource.insert(**insert_kwargs).execute()
        except TypeError as exc:
            if "conferenceDataVersion" not in str(exc):
                raise
        ...
        end_raw = end_obj.get("dateTime") or end_obj.get("date")
        if not isinstance(start_raw, str) or not isinstance(end_raw, str):
            raise ValueError("Google Calendar returned an event without start/end times")

        return CalendarEvent(
            id=str(created.get("id", "")),
            summary=summary,
            start=_ensure_datetime(start_raw),
            end=_ensure_datetime(end_raw),
    ...
    def add_attendees(
        ...
        *,
        ...
        if not attendee_emails:
            raise ValueError("attendee_emails required")

        events_resource = self._service.events()
        event = events_resource.get(calendarId=calendar_id, eventId=event_id).execute()
        existing = list(event.get("attendees") or [])
        known = {str(a.get("email", "")).lower() for a in existing}
        for email in attendee_emails:
            if email.lower() not in known:
                existing.append({"email": email})
        event["attendees"] = existing
        updated = events_resource.patch(
            calendarId=calendar_id,
            eventId=event_id,
            body=event,
        ...
        start_obj = updated.get("start", {})
        end_obj = updated.get("end", {})
        start_raw = start_obj.get("dateTime") or start_obj.get("date")
        end_raw = end_obj.get("dateTime") or end_obj.get("date")
        if not isinstance(start_raw, str) or not isinstance(end_raw, str):
            raise ValueError("Google Calendar returned an event without start/end times")
        ...
        return CalendarEvent(
            id=str(updated.get("id", event_id)),
            summary=str(updated.get("summary") or ""),
            start=_ensure_datetime(start_raw),
            end=_ensure_datetime(end_raw),
            meet_url=_extract_meet_url(updated),
            raw=updated,
        )
