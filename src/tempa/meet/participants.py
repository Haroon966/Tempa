...
def _participant_scraper_script() -> str:
    ...
    from tempa.meet.bot_participants import bot_name_substrings

    escaped = [_re.escape(s) for s in bot_name_substrings()]
    bot_alt = "|".join(escaped) if escaped else "a^"
    return f"""
(() => {{
